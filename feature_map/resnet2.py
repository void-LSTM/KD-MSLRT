...
class Decoder(nn.Module):
    def __init__(self,d_model):
        ...
        self.decoder_linear_FRAME=nn.Linear(48,220)
        self.de_conv1 = nn.Sequential(
                    nn.ConvTranspose2d(1, 3, 3, 1, 1),
                    nn.BatchNorm2d(3),
                    nn.Tanh())
        self.linear=nn.Sequential(nn.Linear(d_model,224*224),
        )
    def forward(self, x):
        x=self.decoder_linear_FRAME(x.transpose(1,2))
        x=x.transpose(1,2)
        b,T,_=x.shape
        x=x.reshape(b,1,T,-1)
        out=self.de_conv1 (x)
...
class MAE_video(nn.Module):
    ...
    def forward(self, x):
        ...
        N, C, T, H, W = x.size()
        out=self.resnet(x)
        out=out.reshape(N,T,out.size(1)).transpose(1, 2)
        out,pred=self.encoder(out)
        #de_out=self.decoder(out)
        de_out=0

        # conv_pred=self.classifier(out).transpose(0,1)
        # conv_pred=F.log_softmax(conv_pred,dim=2)

        
        return de_out,pred#,conv_pred
