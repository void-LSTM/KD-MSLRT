...
class Decoder(nn.Module):
    def __init__(self,d_model):
        ...
        self.de_conv1 = nn.Sequential(
                    nn.ConvTranspose2d(1, 3, 3, 1, 1),
                    nn.BatchNorm2d(3),
                    nn.Tanh())
        self.linear=nn.Sequential(nn.Linear(d_model,2048),
                                  nn.ReLU(),
                                  nn.Linear(2048,4096),
                                  nn.ReLU(),
                                  nn.Linear(4096,224*224),
        )
    def forward(self, x):
        b,T,_=x.shape
        x=x.reshape(b,1,T,-1)
        out=self.de_conv1 (x)
        out=self.linear(out).view(b,3,T,224,224)
        return out
class MAE_video(nn.Module):
    def __init__(self,tgt_vocab_size,d_model,layers):
        super(MAE_video, self).__init__()
        self.resnet=ResNet(BasicBlock,layers,d_model)
        self.encoder=Encoder(tgt_vocab_size,d_model)
        self.decoder=Decoder(d_model)
    def forward(self, x):
        out=self.resnet(x)
        print(out.shape)
        out,pred=self.encoder(out)
        out=self.decoder(out)
        return out,pred
# test=torch.rand((1,3,375,224,224)).cuda()
# model=MAE_video(1193,512,[1, 1, 1, 1]).cuda()
# out,pred=model(test)
